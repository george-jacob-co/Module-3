    ...
    for i in range (0, len(key)):
        if message[i] == " ":
            ...
            if ord(message[i]) + ord(key[i]) <= 155:
                answer += chr (ord(message[i]) + ord(key[i]) - 65)
            else:
                answer += chr (ord(message[i]) + ord(key[i]) - 91)
...
def scytale_decipher(message, shift):
    ...
    for i,m in enumerate(message):
        ...
        for j,n in enumerate (decipher):
            decipher=decipher[:new_letter_index]+m+decipher[new_letter_index+1:]
    return decipher
